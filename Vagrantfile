...
Vagrant.configure(2) do |config|
  ...
  config.vm.hostname = "back-end-vol-eng-b"
...
def setup_environment
  return <<-EOS
    ...
    # Add shortcut alias 'be' to change to project dir
    if ! grep -q 'alias be=' $VAGRANT_HOME/.bashrc; then
      echo >> $VAGRANT_HOME/.bashrc
      echo 'alias be="cd /vagrant"' >> $VAGRANT_HOME/.bashrc
    fi
  EOS
end
